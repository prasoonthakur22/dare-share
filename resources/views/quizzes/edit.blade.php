        ...
        <div class="bg-transparent  px-4 py-12 sm:mx-6 lg:mx-8">
            <div class="max-w-4xl mx-auto">
                ...
                <div class="text-center">
                    <h4 class="text-3xl text-slate-900 font-semibold mt-1 mb-12 pb-1">Update Quizze: <span class="font-bold"> {{ $singleqQuizze->question }} </span></h4>
                </div>
                <div class="text-center">
                ...
                <!-- Quizze Start -->
                <form id="quizzeForm" method="POST" action="{{ route('update.quizze', ['quizze_id' => $singleqQuizze->id] ) }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="rounded-xl px-10 py-10 group shadow-lg hover:shadow-md hover:shadow-slate-800 drop-shadow-xl shadow-slate-800">
                        <div class="my-8">
                            ...
                            <input placeholder="Enter Question" name="question" id="question" type="text" class="form-control input-md  block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-red-600 focus:outline-none" value="{{ $singleqQuizze->question }}" required autocomplete="question" autofocus>
                            @error('question')
                        ...
                        <div class="my-8">
                            ...
                            <input placeholder="Enter Ask Question" name="ask_question" id="ask_question" type="text" class="form-control input-md  block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-red-600 focus:outline-none" value="{{ $singleqQuizze->ask_question }}" required autocomplete="ask_question" autofocus>
                            @error('question')
                            <span class="text-slate-900 italic" role="alert">
                                <span>{{ $message }}</span>
                            </span>
...
<script>
    $(document).ready(function() {
        $('#quizzeForm').on('submit', function(e) {
            e.preventDefault()
            if (true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('update.quizze', ['quizze_id' => $singleqQuizze->id] ) }}",
                    data: {
                        _token: '{{ csrf_token() }}',
                        question: question,
