...
<div class="mx-auto min-h-[90vh]">
    ...
    <div class="p-[1rem] md:p-[5rem] lg:p-[5rem]">
        ...
        <div class="bg-transparent  px-4 py-12 sm:mx-6 lg:mx-8">
            ...
            <div class="max-w-4xl mx-auto glass rounded-3xl group shadow-lg hover:shadow-lg hover:shadow-slate-800 drop-shadow-xl shadow-slate-800 p-10">

                <div class="text-center">
                    <h4 class="text-3xl text-slate-900 font-semibold mt-1 pb-1"><span class="font-bold">{{ $dare->description }}</span> </h4>
                </div>
                <div class="text-center">
                    <h4 class="text-2xl text-slate-900 font-semibold mt-1 pb-1">Dare: {{ $dare->title }}</h4>

                    <h4 class="text-3xl text-slate-900 font-semibold mt-1 pb-1">Quizzee by <span class="font-bold">{{ $isExist->name }}</span></h4>

                </div>

                <form method="POST" action="{{ route('store.responder', ['share_url' => $isExist->share_url]) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="rounded-xl py-10">
                        ...
                        <div class="my-8">
                            <label class="label">
                                <p class="label-text text-slate-900 text-xl mb-2">Name</p>
                                <p class="label-text-alt">Required</p>
                            </label>

                            <input placeholder="Enter your name" id="name" type="text" class="form-control rounded-3xl input-md block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-red-600 focus:outline-none mb-10" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus>
                            @error('name')
